...
function getInstallationId(): string {
    return process.env.INSTALLATION_ID || crypto.randomUUID();
}
...
export async function getLicenseState(): Promise<LicenseState> {
    ...
    if (res.rows.length === 0) {
        ...
        await query('INSERT INTO system_settings (key, value) VALUES ($1, $2)', ['license_state', JSON.stringify(state)]);
    } else {
...
export async function getLicenseStatus(): Promise<LicenseStatusResponse> {
    ...
    return {
        ...
        mode,
        expiresAt: isDev ? new Date(Date.now() + 365*24*60*60*1000).toISOString() : (state.expiresAt !== "1970-01-01T00:00:00Z" ? state.expiresAt : undefined),
...
export async function loadLicensePublicKeyFromDb(): Promise<string | null> {
    try {
        const res = await query('SELECT value FROM system_settings WHERE key = $1', ['license_public_key']);
        if (res.rows.length > 0) {
...
export async function saveLicensePublicKey(input: string): Promise<{ success: boolean; reason?: string }> {
    try {
...
export function verifyLicenseJwt(token: string, publicKeyPem: string, expectedIssuer: string, expectedInstallationId: string): { ok: true; payload: Record<string, unknown> } | { ok: false; reason: string } {
    const parts = token.split('.');
    if (parts.length !== 3) return { ok: false, reason: 'INVALID_FORMAT' };
    const [headerPart, payloadPart, signaturePart] = parts;

    let header: Record<string, unknown>;
    try { header = JSON.parse(base64UrlDecode(headerPart).toString('utf8')); } catch { return { ok: false, reason: 'INVALID_FORMAT' }; }
    if (header.alg !== 'RS256') return { ok: false, reason: 'INVALID_ALG' };

    let payload: Record<string, unknown>;
    try { payload = JSON.parse(base64UrlDecode(payloadPart).toString('utf8')); } catch { return { ok: false, reason: 'INVALID_PAYLOAD' }; }

    const now = Math.floor(Date.now() / 1000);
    const exp = payload.exp as number;
    if (!exp || now >= exp) return { ok: false, reason: 'EXPIRED' };
    if (payload.iss !== undefined && payload.iss !== expectedIssuer) return { ok: false, reason: 'INVALID_ISSUER' };
    if (payload.installation_id !== expectedInstallationId) return { ok: false, reason: 'INVALID_INSTALLATION' };

    const verifier = crypto.createVerify('RSA-SHA256');
    verifier.update(`${headerPart}.${payloadPart}`);
    verifier.end();
    if (!verifier.verify(publicKeyPem, base64UrlDecode(signaturePart))) return { ok: false, reason: 'INVALID_SIGNATURE' };

    return { ok: true, payload };
}

export async function applyLicense(token: string, userId: string): Promise<{ success: boolean, reason?: string }> {
    try {
        const state = await getLicenseState();
        if (!getLicensePublicKeyPem()) {
            await loadLicensePublicKeyFromDb();
        ...
        const result = verifyLicenseJwt(token, publicKeyPem, issuer, state.installationId);
        ...
        const newState: LicenseState = {
            ...
            issuer: issuer,
            expiresAt: result.payload.exp ? new Date((result.payload.exp as number) * 1000).toISOString() : new Date(Date.now() + 365*24*60*60*1000).toISOString(),
            appliedAt: new Date().toISOString(),
            appliedBy: userId,
            licensed: true
        };

        await query('UPDATE system_settings SET value = $1 WHERE key = $2', [JSON.stringify(newState), 'license_state']);
        return { success: true };
