...
export function NotificationCenter() {
  ...
  const [showAll, setShowAll] = useState(false); // Toggle between important/all
  const prevCountRef = useRef(unreadCount);

  // Filter notifications - only show important ones by default
  const importantNotifications = notifications.filter((n) => n.showInBell);
  const displayNotifications = showAll ? notifications : importantNotifications;
  ...
  useEffect(() => {
    ...
    const playBeep = () => {
      ...
      audio.volume = 0.4;
      audio.play().catch((_e) => console.log('Sound play disabled by browser policy'));
    };
  ...
  // Count only important notifications for the bell badge
  const importantUnreadCount = importantNotifications.filter((n) => !n.read).length;
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'error':
        return <AlertCircle className="w-5 h-5 text-rose-600 dark:text-rose-500" />;
      case 'warning':
        return <AlertTriangle className="w-5 h-5 text-amber-500" />;
      case 'info':
        return <Info className="w-5 h-5 text-brand-main" />;
      default:
        return <BellRing className="w-5 h-5 text-text-light" />;
    }
  };
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <Bell
          className={`w-5.5 h-5.5 laptop:w-4.5 laptop:h-4.5 z-10 transition-all duration-300 
                    ${isOpen ? 'rotate-12 scale-110' : ''} 
                    ${importantUnreadCount > 0 && !isOpen ? 'animate-bell-swing' : 'group-hover:animate-bell-swing'}`}
        />
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 mt-3 w-100 bg-panel-bg/80 backdrop-blur-xl border border-white/20 dark:border-white/10 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.2)] z-50 max-h-150 flex flex-col overflow-hidden origin-top-right"
            >
              {/* Header */}
              <div className="flex items-center justify-between px-5 py-4 bg-white/40 dark:bg-black/20 border-b border-border/50 backdrop-blur-md">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-bold text-text-dark tracking-tight">
                    {showAll ? 'Todas Notificações' : 'Notificações'}
                  </h3>
                  {importantUnreadCount > 0 && (
                ...
                <div className="flex gap-1">
                  ...
                  {/* Toggle between Important/All */}
                  <button
                    onClick={() => setShowAll(!showAll)}
                    className={`p-2 rounded-lg transition-all duration-200 ${
                      showAll
                        ? 'text-brand-main bg-brand-main/10'
                        : 'text-text-light hover:text-brand-main hover:bg-brand-main/10'
                    }`}
                    title={showAll ? 'Mostrar somente importantes' : 'Mostrar todas'}
                  >
                  ...
                  <div className="text-center py-16 px-6">
                    ...
                    <p className="text-xs text-text-light/60 mt-1">
                      {showAll
                        ...
                        : 'Sem notificações importantes. Clique no sino para ver todas.'}
                    </p>
                  </div>
                ) : (
                  <div className="py-2">
                    {displayNotifications.map((notification, index) => (
                      <motion.div
                        ...
                        key={notification.id}
                        className={`px-5 py-4 hover:bg-brand-main/5 transition-all cursor-pointer relative group border-l-4 ${
                          !notification.read
                            ? notification.showInBell
                              ? 'border-brand-main bg-brand-main/2'
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-2">
                              <div className="flex items-center gap-2">
                                <h4
                                  className={`font-semibold text-sm transition-colors ${!notification.read ? 'text-text-dark' : 'text-text-light'}`}
                                >
                                  {notification.title}
                                </h4>
                                {/* Category badge for non-important notifications */}
                                {!notification.showInBell && (
                                  <span className="text-[9px] font-medium px-1.5 py-0.5 rounded bg-zinc-200/50 dark:bg-zinc-800/50 text-zinc-500 dark:text-zinc-400">
                                    ação
                                  </span>
                                )}
                              </div>
