...
import { LayoutType, getLayoutConfig, getLayoutGridTemplate } from "@/lib/layouts";
import { MediaType } from "@/lib/store";
import { Upload, Link2, X, Plus, RotateCw, Sun, Moon, Laptop } from "lucide-react";
import { DropZone } from "@/components/upload/drop-zone";

interface ZoneComposerProps {
    layout: LayoutType;
    onZonesChange: (zones: Array<{ id: string; type: MediaType; url: string; name: string; rotation?: number } | null>) => void;
    initialZones?: Array<{ id: string; type: MediaType; url: string; name: string; rotation?: number } | null>;
}
...
export function ZoneComposer({ layout, onZonesChange, initialZones = [] }: ZoneComposerProps) {
    ...
    const [zoneContents, setZoneContents] = useState<Record<number, { type: MediaType; url: string; name: string; rotation?: number; file?: File }>>({});
    ...
    useEffect(() => {
        if (initialZones && initialZones.length > 0) {
            const contents: Record<number, { type: MediaType; url: string; name: string; rotation?: number }> = {};
            initialZones.forEach((zone, index) => {
                if (zone) { // Skip null zones
                    contents[index] = {
                        type: zone.type,
                        url: zone.url,
                        name: zone.name,
    ...
    const handleZoneUpdate = (index: number, content: ZoneContent) => {
        ...
        const zones: Array<{ id: string; type: MediaType; url: string; name: string; rotation?: number } | null> = slotIndices.map(i => {
    ...
    return (
        <div className={`flex flex-col gap-4 ${themeClass}`}>
            <div className="flex justify-between items-center px-1">
                <span className="text-xs font-bold text-text-light uppercase tracking-wider">Visualização de Zonas</span>
            </div>

            <div className={`w-full aspect-video bg-transparent rounded-lg overflow-hidden border border-border shadow-sm p-1 grid ${getGridClass()} gap-1 relative`}>
                            ...
                            {content ? (
                                        ...
                                        <div className="w-full h-full pointer-events-none">
                                            <iframe
                                                ...
                                                src={`https://www.youtube-nocookie.com/embed/${(() => {
                                                    ...
                                                    const match = trimmed.match(/(?:youtu\.be\/|youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|shorts\/|live\/)([^"&?\/\s]{11})/);
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <button
                                                onClick={(e) => {
                                                    ...
                                                    // Notify parent
                                                    const zones: Array<{ id: string; type: MediaType; url: string; name: string } | null> = slotIndices.map(i => {
                                                        const zoneContent = newContents[i];
                                                        if (!zoneContent) return null;
                                                        return {
                                                            id: `zone-${i}`,
                                ...
                                >
                                    <div className="flex flex-col items-center justify-center gap-3 text-text-light group-hover:text-brand-main transition-colors h-full w-full">
                                        ...
                                        <span className="text-[10px] bg-black/5 dark:bg-white/5 px-2 py-0.5 rounded-full mt-1">Zona {index + 1}</span>
                                    </div>
                                </DropZone>
                            )}
                        </div>
            ...
            <div className="p-2 border-t border-border bg-white dark:bg-card relative">
                <div className="relative mb-2">
                    ...
                    <div className="relative flex justify-center text-xs font-bold uppercase tracking-wider"><span className="bg-panel-bg px-2 text-text-light text-[10px]">ou Link Externo</span></div>
                </div>
                ...
                <div className="flex gap-2 items-center">
                    ...
                    {(() => {
                        ...
                        let placeholderText = "Link do YouTube, Google Drive...";
