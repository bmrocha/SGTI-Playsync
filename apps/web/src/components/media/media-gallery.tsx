...
import { 
    Search, Filter, Trash2, Eye, Upload, FileImage, FileVideo, 
    FileAudio, Check, AlertCircle, Loader2, X, Grid, List 
} from "lucide-react";
...
export function MediaGallery({ onSelect, multiSelect = false, className }: MediaGalleryProps) {
    ...
    const fetchFiles = async () => {
        setLoading(true);
        try {
            let url = `/api/media-library?limit=100`;
    ...
    const handleDelete = (id: string) => {
        confirm({
            ...
            message: "Tem certeza que deseja excluir este arquivo permanentemente? Esta ação não pode ser desfeita.",
            type: "danger",
            onConfirm: async () => {
                try {
    ...
    const handleUpload = async (fileList: File[]) => {
        ...
        for (const file of fileList) {
            ...
            const isVideo = file.type?.startsWith('video') || ['mp4', 'webm', 'mov', 'mkv', 'm4v', 'avi', 'wmv', 'flv', 'ogg'].includes(ext);
        ...
        for (let i = 0; i < allowedFiles.length; i++) {
            ...
            try {
                ...
                notifyError("Erro", error instanceof Error ? error.message : `Falha ao enviar ${file.name}`);
            }
        }
    ...
    return (
        <div className={cn("flex flex-col h-full", className)}>
                ...
                <div className="relative flex-1">
                    ...
                    <input 
                        ...
                        className={cn(
                            ...
                            theme === 'dark' ? "bg-white/5 border-white/10 text-white" : "bg-white border-slate-200 text-slate-800 focus:border-brand-main"
                        )}
                    />
                </div>
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                typeFilter === f.id
                                    ...
                                    : (theme === 'dark' ? "bg-white/5 border-white/10 text-slate-400 hover:text-white" : "bg-white border-slate-200 text-slate-500 hover:text-brand-main")
                            )}
                        >
                            ...
                            <div 
                                ...
                                className={cn(
                                    ...
                                    selectedFiles.includes(file.id) 
                                        ...
                                        : (theme === 'dark' ? "bg-white/5 border-white/10 hover:border-white/20" : "bg-white border-slate-200 hover:border-brand-main/30")
                                )}
                            >
                                {/* Preview */}
                                <div className="aspect-square bg-slate-100 dark:bg-black/20 relative overflow-hidden">
                                    {file.mime_type.startsWith('image') ? (
                                        <img src={file.url} alt={file.original_name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                                    ...
                                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                        ...
                                        {onSelect && (
                                            <div className={cn(
                                                ...
                                                selectedFiles.includes(file.id) ? "bg-brand-main border-brand-main" : "border-white"
                                            )}>
                                ...
                                {/* Info */}
                                <div className="p-3">
                                    <h4 className="text-xs font-bold truncate mb-1 text-slate-700 dark:text-slate-200" title={file.original_name}>
                                        {file.original_name}
                                    </h4>
                                    ...
                                    {file.usage && file.usage.length > 0 ? (
                                        <div className="mt-2 flex flex-col gap-1 border-t border-slate-100 dark:border-white/5 pt-2">
                                            <span className="text-[8px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Vinculado a:</span>
                                            <div className="flex flex-wrap gap-1 max-h-20 overflow-y-auto pr-0.5 custom-scrollbar">
                                                {file.usage.map((u, idx) => (
                                                    ...
                                                    >
                                                        <span className="truncate text-[9px]"><strong className="font-bold">Playlist:</strong> {u.playlist_name}</span>
                                                        {u.company_name && (
            ...
            {isUploading && (
                <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center flex-col animate-fadeIn">
                    ...
                    <div className="w-64 h-1 bg-white/20 rounded-full overflow-hidden">
                        <div className="h-full bg-brand-main transition-all duration-300" style={{ width: `${uploadProgress}%` }} />
                    </div>
                </div>
            )}
            ...
            {/* Preview Modal */}
            {previewFile && (
                <div className="fixed inset-0 z-9000 bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn" onClick={() => setPreviewFile(null)}>
                    <div className="relative max-w-5xl w-full max-h-[90vh] flex flex-col items-center" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="w-full flex-1 flex items-center justify-center overflow-hidden rounded-lg bg-black shadow-2xl border border-white/10">
                            {previewFile.mime_type.startsWith('image') ? (
                                <img src={previewFile.url} alt={previewFile.original_name} className="max-w-full max-h-[80vh] object-contain" />
                        ...
                        <div className="mt-4 text-center">
                            ...
                            <p className="text-white/50 text-sm">{formatSize(previewFile.size)} • {new Date(previewFile.uploaded_at).toLocaleString()}</p>
                            ...
                            {previewFile.usage && previewFile.usage.length > 0 ? (
                                <div className="mt-4 max-w-lg mx-auto bg-white/5 border border-white/10 rounded-xl p-3">
                                    <p className="text-white/40 text-[10px] uppercase font-bold tracking-wider mb-2">Playlists e Empresas Vinculadas</p>
                                    <div className="flex flex-wrap gap-1.5 justify-center max-h-32 overflow-y-auto custom-scrollbar">
                                        {previewFile.usage.map((u, idx) => (
                                            ...
                                            >
                                                <span className="text-[11px]"><strong className="font-bold">Playlist:</strong> {u.playlist_name}</span>
                                                {u.company_name && (
                                ...
                                <p className="mt-3 text-white/30 text-xs italic">Este arquivo não está vinculado a nenhuma playlist no momento</p>
                            )}
