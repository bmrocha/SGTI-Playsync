...
export function LayoutSelector({ value, onChange }: LayoutSelectorProps) {
    ...
    const selectedLayout = LAYOUT_CONFIGS.find(l => l.id === value) || LAYOUT_CONFIGS[0];
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {/* Dropdown Menu */}
            {isOpen && (
                <div className="fixed z-[9999] w-64 mt-2 bg-card border border-border rounded-lg shadow-2xl max-h-60 overflow-y-auto animate-scaleIn" style={{
                    ...
                    {LAYOUT_CONFIGS.map((layout) => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full p-3 flex items-center gap-3 hover:bg-brand-main/10 transition-colors border-b border-border last:border-b-0 ${layout.id === value ? 'bg-brand-main/20' : ''
                                    }`}
                            >
                                <div className="w-10 h-10 flex items-center justify-center bg-brand-accent/10 rounded-lg border border-brand-accent/20">
