...
export function Header() {
  ...
  const getTitleFromPath = () => {
    ...
    if (pathname.includes('/sectors')) return 'Setores';
  ...
  return (
    <div
      className="flex h-[70px] laptop:h-[60px] shrink-0 items-center justify-between border-b border-border bg-panel-bg px-8 laptop:px-6 transition-colors duration-300 shadow-sm header-container"
      style={{ boxShadow: 'var(--shadow-sm)' }}
    >
      <div className="flex flex-col gap-0.5">
        <span className="text-[0.8rem] laptop:text-[0.7rem] text-text-light font-normal uppercase tracking-widest">
          PLAYSYNC
        </span>
        <span className="text-[1.2rem] laptop:text-base text-brand-main font-bold m-0">
          {getTitleFromPath()}
        </span>
      </div>
      ...
      <div className="flex items-center gap-4 laptop:gap-3">
        ...
        >
          {theme === 'light' ? (
            <Moon
              className="w-5 h-5 laptop:w-4 laptop:h-4 transition-transform duration-500 group-hover:rotate-[-15deg] group-hover:scale-110 origin-center"
              data-hover="glow"
            />
          ) : (
            <Sun
              className="w-5 h-5 laptop:w-4 laptop:h-4 transition-transform duration-500 group-hover:rotate-180 group-hover:scale-110"
              data-hover="glow"
            />
          )}
        </button>
        ...
        >
          {isLoggingOut ? (
            ...
            <LogOut
              className="w-5 h-5 laptop:w-4 laptop:h-4 transition-transform duration-500 group-hover:rotate-[-5deg] group-hover:scale-110 origin-center"
              data-hover="glow"
            />
          )}
        </button>
        ...
        >
          ...
          <div
            className="w-10 h-10 laptop:w-8 laptop:h-8 rounded-full bg-brand-main/10 border-2 border-brand-main/20 flex items-center justify-center text-brand-main group-hover:bg-brand-main group-hover:text-white group-hover:border-brand-main transition-all duration-300 overflow-hidden group-hover:scale-110 relative"
            data-hover="lift"
          >
            {user?.avatar ? (
              <Image
