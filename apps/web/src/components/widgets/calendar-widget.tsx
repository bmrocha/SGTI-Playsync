...
export function CalendarWidget({ config = {}, className }: CalendarWidgetProps) {
    ...
    const monthHighlights = Array.from(new Map(
        allHolidays
            .filter((h) => {
    ...
    useEffect(() => {
        ...
        const t = setInterval(
            () => setActiveIndex((prev) => (prev + 1) % monthHighlights.length),
            ms
        );
        return () => clearInterval(t);
    }, [config.carouselEnabled, config.carouselInterval, monthHighlights.length]);

    useEffect(() => { setActiveIndex(0); }, [monthHighlights.length]);

    const currentItem = monthHighlights[activeIndex];
        ...
        <div className={cn('w-full h-full overflow-hidden', className)}
            ...
            <div className="overflow-hidden flex flex-col justify-center px-6 pt-3 pb-2">
                ...
                <div className="flex items-center gap-4">
                    <span className={cn(
                        'text-5xl laptop:text-7xl font-black tracking-tighter leading-none flex-shrink-0',
            ...
            <div className="overflow-hidden flex flex-col border-t-2 border-slate-200/60 dark:border-white/10 px-6 pt-1 pb-2">
                ...
                {config.carouselEnabled ? (
                            ...
                            {monthHighlights.length > 0 ? (
                                <div className="w-full flex flex-col gap-2 transition-all duration-700">
                                    {[
                                        ...
                                        monthHighlights.length > 2 ? monthHighlights[(activeIndex + 1) % monthHighlights.length] : null
                                    ].filter(Boolean).map((item: any, subIdx) => (
                                        <div key={`${item.date}-${item.name}`} className="flex items-stretch gap-0 w-full rounded-2xl bg-brand-main/10 dark:bg-brand-main/25 border-2 border-brand-main/20 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-500">
                                            {/* Lado Esquerdo: Data */}
                                            <div className="flex-shrink-0 flex flex-col items-center justify-center bg-brand-main/15 px-4 py-2.5 border-r-2 border-brand-main/10 min-w-[75px]">
                                                <span className="text-[10px] laptop:text-xs font-black uppercase leading-none mb-1 text-brand-main opacity-80">
                                                    {format(new Date(item.date + 'T12:00:00'), 'MMM', { locale: ptBR }).replace('.', '')}
                                                </span>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2">
                        {monthHighlights.length > 0 ? monthHighlights.map((h, i) => {
                            const hDate = new Date(h.date + 'T12:00:00');
                            return (
                                <div key={i} className="flex items-center gap-4 p-3 rounded-2xl bg-brand-main/5 dark:bg-white/5">
