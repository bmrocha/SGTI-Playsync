...
export function NewsTickerWidget({ config, className }: NewsTickerWidgetProps) {
    ...
    const isDark = effectiveTheme === 'dark' || (config.backgroundColor && config.backgroundColor.toLowerCase() === '#000000');
    ...
    const fetchRssNews = async () => {
        ...
        try {
            ...
            let items = Array.from(xmlDoc.querySelectorAll("item > title")).map(el => el.textContent || "");

            // Fallback for Atom feeds (entry > title)
            if (items.length === 0) {
                items = Array.from(xmlDoc.querySelectorAll("entry > title")).map(el => el.textContent || "");
            }
            ...
            console.error('[NewsTicker] Error loading RSS:', err);
    ...
    const truncateUrl = (url: string) => {
        try {
            ...
            return urlObj.hostname + (urlObj.pathname.length > 10 ? urlObj.pathname.substring(0, 15) + '...' : urlObj.pathname);
        } catch (e) {
    ...
    return (
        <div
            ...
            style={{
                backgroundColor: hasCustomBackground && !isOpaqueThemeBackground ? config.backgroundColor : undefined,
        ...
        >
            ...
            {config.showClock !== false && (
                ...
                >
                    <Clock className={cn(hasCustomColors ? "opacity-80" : "text-brand-main opacity-80", density === 'compact' ? "w-8 h-8" : "w-12 h-12")} />
                    <div className="flex flex-col leading-none justify-center">
                        <span
                            className={cn(
                                "font-black tracking-tighter",
                                hasCustomColors ? "" : "text-slate-900 dark:text-white",
                                density === 'compact' ? "text-[clamp(24px,2.4vw,32px)]" : "text-5xl laptop:text-7xl"
                            )}
                        >
                            {formatTime()}
                        </span>
            ...
            <div
                className={cn(
                    ...
                    hasCustomColors ? "border-white/20" : "text-brand-main border-brand-main/20 dark:border-white/10",
                    density === 'compact' ? "text-[clamp(12px,1.2vw,14px)] h-6" : "text-2xl laptop:text-3xl h-16"
                )}
            >
            ...
            <div className="flex-1 overflow-hidden relative h-full flex items-center">
                <div
                    ...
                    ref={tickerRef}
                    className={cn("flex whitespace-nowrap", "animate-scroll-ticker", density === 'compact' ? "gap-24" : "gap-48")}
                    style={{
                    ...
                    {newsItems.map((news, idx) => (
                        <span
                            ...
                            className={cn(
                                ...
                                density === 'compact' ? "text-[clamp(16px,1.6vw,20px)] gap-6" : "text-[26px] laptop:text-[34px] gap-20"
                            )}
                        >
                            {news}
                            <span
