...
export function MessageWidget({ config, className }: MessageWidgetProps) {
  ...
  const Icon =
    config.type === 'warning' ? AlertCircle : config.type === 'campaign' ? Megaphone : Info;
  ...
  const _bgIsThemeTint = isOpaqueThemeBackground;
  ...
  const messageFontSize =
    config.display === 'fullscreen'
      ...
      : typeof config.fontSize === 'number'
        ...
        : undefined;
  const messageFontWeight = config.display === 'fullscreen' ? undefined : config.fontWeight;
  ...
  const selectedAnimation =
    animations[config.animation as keyof typeof animations] || animations.none;
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        !textIsCustom &&
          (config.theme === 'light' ? 'text-slate-900' : 'text-slate-800 dark:text-white'),
    ...
    >
      ...
      <div className="absolute left-8 flex items-center gap-3 shrink-0 z-10 select-none">
      ...
      <div className="flex items-center gap-5 max-w-[75%] min-w-0">
        <div
          className={cn(
            'w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm transition-transform duration-500 hover:scale-110',
            config.type === 'warning'
              ? 'bg-red-500/20 text-red-600 dark:text-red-400'
              : config.type === 'campaign'
                ? 'bg-brand-main/20 text-brand-main dark:text-brand-main'
        ...
        <p
          className={cn(
            ...
            config.display === 'fullscreen' &&
              'text-5xl font-black italic uppercase whitespace-normal!',
          )}
          style={{
