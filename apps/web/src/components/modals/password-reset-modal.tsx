...
export function PasswordResetModal({
    ...
    const handleResetPassword = async () => {
        ...
        if (password === suggestedPassword) {
            notifyError("Erro", "Crie uma senha manualmente. A senha sugerida não pode ser utilizada.");
            return;
        }
        ...
        if (!isPasswordValid) {
            notifyError("Erro", "A senha deve ter no mínimo 12 caracteres, incluindo maiúscula, minúscula, número e caractere especial.");
            return;
        }
            ...
            <div onKeyDown={(e) => checkCapsLock(e as any)}>
                ...
                <div className="p-3 bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-900/30 rounded-lg">
                    <div className="flex items-center justify-between mb-1">
                        <p className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">Senha sugerida</p>
                        <button
                            type="button"
                    ...
                    <p className="text-xs font-mono text-emerald-800 dark:text-emerald-300 break-all select-all">{suggestedPassword}</p>
                    <p className="text-[9px] text-emerald-600/60 dark:text-emerald-400/60 mt-1">Apenas referência — crie sua própria senha manualmente.</p>
                </div>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <div className="relative">
                            ...
                            />
                            {password.length > 0 && (
                                <span className={`absolute right-10 top-1/2 -translate-y-1/2 text-[10px] font-bold ${
                            ...
                            <div className="space-y-1 mt-2">
                                {[
                                    ...
                                    <div key={i} className="flex items-center gap-1.5">
                                        ...
                                        <span className={`text-[11px] font-medium ${req.valid ? 'text-emerald-500' : 'text-text-light'}`}>
                                            {req.label}
                                        </span>
                                    </div>
                                ))}
                    ...
                    <div>
                        ...
                        {confirmPassword.length > 0 && (
                            <div className="flex items-center gap-1.5 mt-1">
                                ...
                                <span className={`text-[11px] font-medium ${confirmPassword === password && isPasswordValid ? 'text-emerald-500' : 'text-text-light'}`}>
                                    Senhas coincidem
                                </span>
                            </div>
                        )}
