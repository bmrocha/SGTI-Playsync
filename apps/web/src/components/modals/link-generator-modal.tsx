...
import { X, Link2, Copy, Check, Sun, Moon, Loader2, Save, ShieldAlert } from "lucide-react";
...
export function LinkGeneratorModal({
    isOpen, onClose, items, playlistName, companyName, playlistDbId, companyId
    ...
    useEffect(() => {
        ...
        fetch('/api/playlist-links', {
            ...
            body: JSON.stringify({ companyName, playlistName, playlistId: playlistDbId, companyId, items: safeItems }),
        })
            .then(r => r.json())
            .then(async data => {
                if (!data.id) return;
                ...
                setSavedTheme(t); setSavedColor(c);
                setDraftTheme(t); setDraftColor(c);
            })
    ...
    const selectedColorLabel = LINK_COLORS.find(c => c.hex === draftColor)?.label ?? "Personalizada";
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await fetch('/api/playlist-links', {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    companyName, playlistName, playlistId: playlistDbId, companyId,
                    items: safeItems, theme: draftTheme, primaryColor: draftColor,
                }),
            });
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-6000 flex items-center justify-center p-4">
            <div className="bg-panel-bg rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-border">
                ...
                <div className="p-6 space-y-5">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <span className="text-xs font-bold text-text-light uppercase tracking-wider">Tema</span>
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                <span className="text-xs font-bold text-text-light uppercase tracking-wider">Cor Principal</span>
                    ...
                    <div className="space-y-1.5">
                        ...
                        {removedCount > 0 && (
                            <p className="text-[10px] text-amber-500">
                                ⚠️ {removedCount} arquivo(s) local/locais removido(s) do link (arquivos locais não são compartilháveis).
                            </p>
                        )}
                    </div>
                    ...
                    <div className="flex items-start gap-3 p-3 bg-red-500/8 border border-red-500/20 rounded-xl">
                        ...
                        <p className="text-xs text-text-dark leading-relaxed">
                            <strong className="text-red-500">Atenção:</strong> Qualquer pessoa com este link ou ID poderá visualizar a playlist.{" "}
