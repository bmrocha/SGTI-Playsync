...
import { Calendar, Check, ChevronLeft, ChevronRight, Clock, Layout, Link2, Monitor, Play, Trash2, Upload, X, RotateCw, Sparkles, Video, Youtube, FileType, HardDrive, Plus, Minus, Globe } from "lucide-react";
...
type YoutubeQuality = 'auto' | 'hd2160' | 'hd1440' | 'hd1080' | 'hd720' | 'large' | 'medium' | 'small' | 'tiny';
...
export function MediaConfigModal({ isOpen, onClose, onSave, onAutoSave, initialItem }: MediaConfigModalProps) {
    ...
    // Content State
    // For Multi-zone
    const [zoneDraft, setZoneDraft] = useState<Array<{ id: string; type: MediaType; url: string; name: string; file?: File } | null>>([]);
    ...
    useEffect(() => {
        ...
        const savedTab = localStorage.getItem('mediaConfigActiveTab') as 'content' | 'layout' | 'schedule' | null;
            ...
            if (!isInitialized || lastItemIdRef.current !== currentId) {
                if (initialItem) {
                    ...
                    if (initialItem.layout !== 'single' && initialItem.zones) {
                        ...
                        setSingleUrl(initialItem.type === 'youtube' || initialItem.type === 'video' || initialItem.type === 'image' ? initialItem.url : "");
    ...
    const constructItem = (overrideUrl?: string, overrideZones?: any[]): MediaItem => {
        ...
        if (finalLayout === 'single') {
            if (singleFile) {
                const isVideoFile = singleFile.type.startsWith('video') || 
        ...
        return {
            ...
            name: initialItem?.name || (singleFile?.name || (currentUrl ? 'URL Media' : (layoutTemplateId ? 'Layout Template' : `Slide ${layout}`))),
            duration,
            rotation,
            layout: finalLayout,
    ...
    useEffect(() => {
        ...
        duration, rotation, layout, 
        scheduleEnabled, daysOfWeek, startDate, endDate, startTime, endTime, allDay,
        zoneDraft, singleUrl, singlePreview, layoutTemplateId, regionConfig, urlType,
        isInitialized, initialItem, onAutoSave, singleFile
    ]);
    ...
    const handleSave = async () => {
        ...
        if (finalLayout === 'single' && singleFile) {
            try {
                ...
                notifyError("Erro no upload", error instanceof Error ? error.message : "Falha ao enviar arquivo. Tente novamente.");
        ...
        if (layout !== 'single' && finalZones.some(z => z !== null)) {
            try {
                ...
                notifyError("Erro no upload", error instanceof Error ? `Falha ao enviar imagens das zonas: ${error.message}` : "Falha ao enviar imagens das zonas. Verifique o tamanho dos arquivos.");
                setIsUploading(false);
                return;
            }
        }
        ...
        if (finalLayout === 'single' && finalUrl && finalUrl.startsWith('blob:')) {
             ...
             notifyError("Erro de Persistência", "A mídia ainda é um arquivo local temporário. Tente fazer o upload novamente.");
             setIsUploading(false);
             return;
        }
    ...
    const handleSingleFile = (files: File[]) => {
        if (files.length > 0) {
            ...
            if (file.type.startsWith('video') || file.name.match(/\.(mp4|webm|ogg|mov|mkv|m4v|avi|wmv|flv)$/i)) {
                setUrlType('video');
            } else {
                setUrlType('auto'); 
            }
    ...
    const getYoutubeEmbedUrl = (url: string, config?: Partial<YoutubePlayerConfig>) => {
        ...
        const originParam = typeof window !== 'undefined'
            ...
            : '';
        const start = Number.isFinite(config?.startSeconds as number) ? `&start=${Math.max(0, Math.floor(config!.startSeconds!))}` : '';
        ...
        const ccLang = config?.captionsLang ? `&cc_lang_pref=${encodeURIComponent(config.captionsLang)}` : '';
    ...
    const getPreviewContent = () => {
        ...
        if (isVid) {
             return (
                <video src={singlePreview} className="w-full h-full object-contain" style={{ transform: `rotate(${rotation}deg)` }} controls />
            );
        }
            ...
            <div className={cn(
                ...
                <div className={cn(
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className={cn("text-sm font-bold uppercase tracking-tight", theme === 'dark' ? "text-white" : "text-slate-800")}>
                ...
                <div className={cn(
                    ...
                    {isUploading && (
                        <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-8 animate-fadeIn">
                            ...
                            <p className="text-sm text-white font-bold mb-2">Enviando Mídia ({uploadProgress}%)</p>
                                ...
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        activeTab === tab.id
                                            ...
                                            : (theme === 'dark' ? "text-slate-400 hover:bg-white/5" : "text-slate-500 hover:text-brand-main hover:bg-slate-50")
                                    )}
                                >
                                    ...
                                    <div className={cn(
                                            ...
                                            {singlePreview ? (
                                                <div>
                                                    <div className="relative w-full aspect-video bg-slate-100 dark:bg-black/40 rounded-lg overflow-hidden group border border-slate-200 dark:border-white/10 shadow-inner">
                                                        {layoutTemplateId && LAYOUT_TEMPLATES.find(t => t.id === layoutTemplateId) ? (
                                                            <LayoutRenderer 
                                                        ...
                                                        <div className="absolute inset-0 bg-linear-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-4 gap-3 z-50">
                                                            <button onClick={() => setRotation((prev) => (prev + 90) % 360)} className="px-3 py-1.5 bg-white/15 hover:bg-white/25 rounded-full text-white backdrop-blur-md border border-white/10 text-xs font-bold flex items-center gap-1.5 transition-all" title="Girar"><RotateCw className="w-3.5 h-3.5" /> {rotation}°</button>
                                                            <button onClick={() => { setSingleFile(null); setSinglePreview(""); setSingleUrl(""); setRotation(0); }} className="px-3 py-1.5 bg-red-500/70 hover:bg-red-500 rounded-full text-white text-xs font-bold flex items-center gap-1.5 transition-all" title="Remover"><Trash2 className="w-3.5 h-3.5" /> Remover</button>
                                                        </div>
                                                    </div>
                                                ...
                                                <div className="flex flex-col gap-3">
                                                    <DropZone onFilesAccepted={handleSingleFile} multiple={false} className="h-40 w-full" variant="default" />
                                                    ...
                                                    <button 
                                                        ...
                                                        className={cn(
                                                            ...
                                                            theme === 'dark' ? "border-white/10 hover:border-brand-main text-slate-400 hover:text-white" : "border-slate-200 hover:border-brand-main text-slate-500 hover:text-brand-main"
                                                        )}
                                                    >
                                                        <HardDrive className="w-5 h-5" />
                                                        <span className="text-xs font-bold uppercase tracking-wider">Selecionar da Galeria</span>
                                                    </button>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className={cn(
                                            ...
                                            <div className="flex gap-2 items-center">
                                                <div className={cn("w-10 h-10 flex items-center justify-center rounded-lg border", theme === 'dark' ? "bg-white/5 border-white/5 text-white/40" : "bg-white border-slate-200 text-slate-400")}><Link2 className="w-4 h-4" /></div>
                                                <input
                                                    type="text"
                                                    value={singleUrl}
                                                    ...
                                                    className={cn(
                                                        ...
                                                        theme === 'dark' ? "bg-black/20 border-white/10 text-white" : "bg-white border-slate-200 text-slate-800 focus:border-brand-main"
                                                    )}
                                                />
                                                {/* Type Selector */}
                                                ...
                                                <div className={cn(
                                                    ...
                                                    <details className="mt-3" open>
                                                        <summary className={cn(
                                                            ...
                                                            theme === 'dark' ? "bg-black/20 border-white/10 text-white/80" : "bg-white border-slate-200 text-slate-700"
                                                        )}>
                                                            <div className="flex items-center gap-2">
                                                                <Youtube className="w-4 h-4 text-red-500" />
                                                                <span className="text-[11px] font-bold uppercase tracking-wider">Configurações YouTube</span>
                                                                <span className="text-[10px] text-slate-400">{videoId ? `ID: ${videoId}` : 'URL inválida'}</span>
                                                            </div>
                                                        </summary>
                                                        ...
                                                        <div className={cn(
                                                            ...
                                                            <div className="grid grid-cols-2 gap-3">
                                                                <div>
                                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Início (segundos)</label>
                                                                    <input
                                                                        type="number"
                                                                        value={ytConfig.startSeconds ?? ''}
                                                                        onChange={(e) => {
                                                                            const v = e.target.value === '' ? undefined : Math.max(0, Math.floor(Number(e.target.value)));
                                                                            updateYoutubeConfig({ startSeconds: Number.isFinite(v as number) ? (v as number) : undefined });
                                                                        }}
                                                                        className={cn(
                                                                ...
                                                                <div>
                                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Volume (0–100)</label>
                                                                    <input
                                                                        type="number"
                                                                        ...
                                                                        onChange={(e) => {
                                                                            ...
                                                                            updateYoutubeConfig({ volume: Number.isFinite(v) ? v : undefined });
                                                                        }}
                                                                        className={cn(
                                                                            "w-full h-10 px-3 border rounded-lg text-sm outline-none transition-all",
                                                                            theme === 'dark' ? "bg-black/20 border-white/10 text-white" : "bg-white border-slate-200 text-slate-800 focus:border-brand-main"
                                                                ...
                                                                <div>
                                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Velocidade</label>
                                                                    <select
                                                                        value={ytConfig.playbackRate ?? 1}
                                                                        onChange={(e) => updateYoutubeConfig({ playbackRate: Number(e.target.value) })}
                                                                        className={cn(
                                                                            "w-full h-10 px-3 border rounded-lg text-sm outline-none transition-all",
                                                                            theme === 'dark' ? "bg-black/20 border-white/10 text-white" : "bg-white border-slate-200 text-slate-800 focus:border-brand-main"
                                                                        )}
                                                                ...
                                                                <div>
                                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Qualidade</label>
                                                                    <select
                                                                        value={ytConfig.quality || 'auto'}
                                                                        onChange={(e) => updateYoutubeConfig({ quality: e.target.value as YoutubeQuality })}
                                                                        className={cn(
                                                                            "w-full h-10 px-3 border rounded-lg text-sm outline-none transition-all",
                                                                            theme === 'dark' ? "bg-black/20 border-white/10 text-white" : "bg-white border-slate-200 text-slate-800 focus:border-brand-main"
                                                                        )}
                                                                    >
                                                                        {['auto', 'hd2160', 'hd1440', 'hd1080', 'hd720', 'large', 'medium', 'small', 'tiny'].map(v => (
                                                                ...
                                                                <div className="col-span-2 grid grid-cols-2 gap-3">
                                                                    <label className={cn(
                                                                        ...
                                                                        theme === 'dark' ? "bg-black/20 border-white/10 hover:bg-white/5" : "bg-white border-slate-200 hover:border-brand-main"
                                                                    )}>
                                                                        <span className={cn("text-[10px] font-bold uppercase tracking-wider", theme === 'dark' ? "text-white/70" : "text-slate-600")}>Mudo</span>
                                                                        <input
                                                                            type="checkbox"
                                                                            checked={ytConfig.muted ?? false}
                                                                            onChange={(e) => updateYoutubeConfig({ muted: e.target.checked })}
                                                                    ...
                                                                    <label className={cn(
                                                                        ...
                                                                        <span className={cn("text-[10px] font-bold uppercase tracking-wider", theme === 'dark' ? "text-white/70" : "text-slate-600")}>Controles</span>
                                                                        <input
                                                                            type="checkbox"
                                                                            checked={ytConfig.controls ?? true}
                                                                            onChange={(e) => updateYoutubeConfig({ controls: e.target.checked })}
                                                                            className="w-4 h-4 rounded accent-brand-main text-brand-main focus:ring-0"
                                                                        />
                                                                    </label>
                                                                </div>
                                                                <div className="col-span-2 grid grid-cols-2 gap-3">
                                                                    <label className={cn(
                                                                        ...
                                                                        <span className={cn("text-[10px] font-bold uppercase tracking-wider", theme === 'dark' ? "text-white/70" : "text-slate-600")}>Legendas</span>
                                                                        <input
                                                                            type="checkbox"
                                                                            checked={ytConfig.captionsEnabled ?? false}
                                                                            onChange={(e) => updateYoutubeConfig({ captionsEnabled: e.target.checked })}
                                                                            className="w-4 h-4 rounded accent-brand-main text-brand-main focus:ring-0"
                                                                        />
                                                                    </label>
                                                                    <div>
                                                                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Idioma</label>
                                                                        <input
                                                                            type="text"
                                                                            value={ytConfig.captionsLang ?? ''}
                                                                            onChange={(e) => updateYoutubeConfig({ captionsLang: e.target.value || undefined })}
                                ...
                                <details className="pt-4 border-t border-slate-200 dark:border-white/5" open>
                                    <summary className="list-none cursor-pointer select-none">
                                        <div className="flex items-center gap-3 mb-4">
                                            ...
                                            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500">Configurar Template</h3>
                                        </div>
                                    </summary>
                                    <div className="grid grid-cols-1 gap-4 pb-1">
                                        {LAYOUT_TEMPLATES.find(t => t.id === layoutTemplateId)?.regions.filter(r => r.widgetType !== 'media').map(region => {
                                            const currentConfig = { ...region.config, ...(regionConfig[region.id] || {}) };
                                            return (
                                                <div key={region.id} className={cn(
                                                    ...
                                                    <div className="flex items-center gap-3 mb-4 border-b border-slate-100 dark:border-white/5 pb-2">
                                                        <span className={cn("px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider text-white", 
                                                            region.type === 'sidebar' ? 'bg-blue-500' : 
                                                            region.type === 'ticker' ? 'bg-green-500' : 'bg-brand-main'
                                                        )}>
                                                            {region.type}
                                                        </span>
                                                        <span className="text-xs font-bold uppercase text-slate-500">{region.widgetType}</span>
                                                    </div>
                                                        ...
                                                        <div className="space-y-3">
                                                            <div>
                                                                ...
                                                                <textarea
                                                                    ...
                                                                    onChange={(e) => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, text: e.target.value } }))}
                                                                    className={cn(
                                                                        "w-full px-3 py-2 border rounded-lg text-sm outline-none transition-all min-h-15 resize-none",
                                                                        theme === 'dark' ? "bg-black/20 border-white/10 text-white" : "bg-white border-slate-200 text-slate-800"
                                                                    )}
                                                                />
                                                            </div>
                                                            <div>
                                                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Estilo</label>
                                                                <div className="flex gap-2">
                                                                    {['campaign', 'info', 'warning'].map(type => (
                                                                        <button
                                                                            key={type}
                                                                            onClick={() => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, type: type } }))}
                                                                            className={cn(
                                                                                "px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider border transition-all flex-1",
                                                                                (currentConfig.type === type || (!currentConfig.type && type === 'campaign'))
                                                        ...
                                                        <div className="space-y-3">
                                                            <div>
                                                                ...
                                                                <LocationAutocomplete
                                                                    ...
                                                                    onChange={(val) => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, location: val } }))}
                                                            ...
                                                            <div className="grid grid-cols-2 gap-2">
                                                                <label className="flex items-center justify-between p-2 border border-slate-200 dark:border-white/10 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5">
                                                                    <span className="text-[10px] font-bold uppercase text-slate-500">Dados em Tempo Real</span>
                                                                    <input type="checkbox" checked={currentConfig.useApi !== false} onChange={(e) => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, useApi: e.target.checked } }))} className="w-3 h-3 rounded accent-brand-main text-brand-main focus:ring-0" />
                                                                </label>
                                                                <label className="flex items-center justify-between p-2 border border-slate-200 dark:border-white/10 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5">
                                                                    <span className="text-[10px] font-bold uppercase text-slate-500">Previsão</span>
                                                                    <input type="checkbox" checked={currentConfig.showForecast !== false} onChange={(e) => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, showForecast: e.target.checked } }))} className="w-3 h-3 rounded accent-brand-main text-brand-main focus:ring-0" />
                                                                </label>
                                                                <label className="flex items-center justify-between p-2 border border-slate-200 dark:border-white/10 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5">
                                                                    <span className="text-[10px] font-bold uppercase text-slate-500">Humidade</span>
                                                                    <input type="checkbox" checked={currentConfig.showHumidity !== false} onChange={(e) => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, showHumidity: e.target.checked } }))} className="w-3 h-3 rounded accent-brand-main text-brand-main focus:ring-0" />
                                                                </label>
                                                            </div>
                                                    ...
                                                    {region.widgetType === 'news' && (
                                                        <div className="space-y-3">
                                                            <div>
                                                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Fonte RSS (Opcional)</label>
                                                                <input
                                                                    type="text"
                                                            ...
                                                            <div>
                                                                <div className="flex items-center justify-between mb-2">
                                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Manchetes Manuais</label>
                                                                    <button
                                                                        type="button"
                                                                        onClick={() => {
                                                                            const items = currentConfig.newsItems || [];
                                                                            setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, newsItems: [...items, ''] } }));
                                                                        }}
                                                                ...
                                                                <div className="space-y-2">
                                                                    {(currentConfig.newsItems || []).map((item: string, idx: number) => (
                                                                        <div key={idx} className="flex gap-2">
                                                                            <input
                                                                                ...
                                                                                className={cn(
                                                                                     ...
                                                                                     theme === 'dark' ? "bg-black/20 text-white" : "bg-white text-slate-800"
                                                                                 )}
                                                                            />
                                                                            <button
                                                                                onClick={() => {
                                                            ...
                                                            <div className="grid grid-cols-2 gap-2">
                                                                <label className="flex items-center justify-between p-2 border border-slate-200 dark:border-white/10 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5">
                                                                    <span className="text-[10px] font-bold uppercase text-slate-500">Exibir Relógio</span>
                                                                    <input type="checkbox" checked={currentConfig.showClock !== false} onChange={(e) => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, showClock: e.target.checked } }))} className="w-3 h-3 rounded accent-brand-main text-brand-main focus:ring-0" />
                                                                </label>
                                                            </div>
                                                    ...
                                                    {region.widgetType === 'clock' && (
                                                        <div className="space-y-3">
                                                            <div className="grid grid-cols-2 gap-2">
                                                                <label className="flex items-center justify-between p-2 border border-slate-200 dark:border-white/10 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5">
                                                                    <span className="text-[10px] font-bold uppercase text-slate-500">Exibir Hora</span>
                                                                    <input type="checkbox" checked={currentConfig.showTime !== false} onChange={(e) => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, showTime: e.target.checked } }))} className="w-3 h-3 rounded accent-brand-main text-brand-main focus:ring-0" />
                                                                </label>
                                                                <label className="flex items-center justify-between p-2 border border-slate-200 dark:border-white/10 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5">
                                                                    <span className="text-[10px] font-bold uppercase text-slate-500">Exibir Data</span>
                                                                    <input type="checkbox" checked={currentConfig.showDate !== false} onChange={(e) => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, showDate: e.target.checked } }))} className="w-3 h-3 rounded accent-brand-main text-brand-main focus:ring-0" />
                                                                </label>
                                                                <label className="flex items-center justify-between p-2 border border-slate-200 dark:border-white/10 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5">
                                                                    <span className="text-[10px] font-bold uppercase text-slate-500">Segundos</span>
                                                                    <input type="checkbox" checked={currentConfig.showSeconds !== false} onChange={(e) => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, showSeconds: e.target.checked } }))} className="w-3 h-3 rounded accent-brand-main text-brand-main focus:ring-0" />
                                                                </label>
                                                                <label className="flex items-center justify-between p-2 border border-slate-200 dark:border-white/10 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5">
                                                                    <span className="text-[10px] font-bold uppercase text-slate-500">Formato 24h</span>
                                                                    <input type="checkbox" checked={currentConfig.format !== '12h'} onChange={(e) => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, format: e.target.checked ? '24h' : '12h' } }))} className="w-3 h-3 rounded accent-brand-main text-brand-main focus:ring-0" />
                                                                </label>
                                                            </div>
                                                        </div>
                                                    )}
                                                    ...
                                                    {region.widgetType === 'calendar' && (
                                                        <div className="space-y-3">
                                                            <div className="grid grid-cols-1 gap-2">
                                                                <label className="flex items-center justify-between p-2 border border-slate-200 dark:border-white/10 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5">
                                                                    <span className="text-[10px] font-bold uppercase text-slate-500">Destacar Hoje</span>
                                                                    <input type="checkbox" checked={currentConfig.highlightToday !== false} onChange={(e) => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, highlightToday: e.target.checked } }))} className="w-3 h-3 rounded accent-brand-main text-brand-main focus:ring-0" />
                                                                </label>
                                                            </div>

                                                            <div className="grid grid-cols-2 gap-2 mt-2">
                                                                <label className="flex items-center justify-between p-2 border border-slate-200 dark:border-white/10 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-white/5">
                                                                    <span className="text-[10px] font-bold uppercase text-slate-500">Ativar Carrossel</span>
                                                                    <input type="checkbox" checked={currentConfig.carouselEnabled === true} onChange={(e) => setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, carouselEnabled: e.target.checked } }))} className="w-3 h-3 rounded accent-brand-main text-brand-main focus:ring-0" />
                                                                </label>
                                                                <div className="flex items-center justify-between p-2 border border-slate-200 dark:border-white/10 rounded-lg">
                                                                    <span className="text-[10px] font-bold uppercase text-slate-500">Intervalo (s)</span>
                                                                    <input 
                                                                        type="text" 
                                                                        maxLength={3}
                                                            ...
                                                            <div>
                                                                <div className="flex flex-col gap-2 mb-2 mt-2">
                                                                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Eventos do Calendário</label>
                                                                    <div className="flex gap-2">
                                                                        <button
                                                                            type="button"
                                                                            onClick={() => {
                                                                                ...
                                                                                setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, customEvents: [...items, { date: today, name: '' }] } }));
                                                                            }}
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            onClick={() => {
                                                                                ...
                                                                                const holidays = getBrazilianHolidays(year).filter(h => {
                                                                                    const hDate = new Date(h.date + 'T12:00:00');
                                                                                    return hDate.getMonth() === month;
                                                                                });
                                                                                const currentEvents = currentConfig.customEvents || [];
                                                                                const newEvents = holidays.filter(h => !currentEvents.some((e: any) => e.date === h.date && e.name === h.name));
                                                                                
                                                                                if (newEvents.length === 0) {
                                                                                    notifySuccess("Datas Atualizadas", "As datas do mês já foram geradas!");
                                                                                    return;
                                                                                }
                                                                                
                                                                                setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, customEvents: [...currentEvents, ...newEvents] } }));
                                                                                notifySuccess("Geração Concluída", `${newEvents.length} datas geradas para este mês!`);
                                                                            }}
                                                                ...
                                                                <div className="space-y-2">
                                                                    {(currentConfig.customEvents || []).map((event: {date: string, name: string}, idx: number) => (
                                                                        <div key={idx} className="flex gap-2">
                                                                            <input
                                                                                type="date"
                                                                                value={event.date}
                                                                                onChange={(e) => {
                                                                                    ...
                                                                                    setRegionConfig(prev => ({ ...prev, [region.id]: { ...currentConfig, customEvents: items } }));
                                                                                }}
                                                                                className={cn(
                            ...
                            {layout !== 'single' && (
                                ...
                                <div className="animate-fadeIn">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Editor de Camadas</span>
                                    </div>
                                    <div className={cn(
                                        "rounded-xl border p-1 transition-all",
                                        theme === 'dark' ? "border-white/5 bg-black/20" : "border-slate-200 bg-white"
                                    )}>
                                        <ZoneComposer key={`${layout}-${composerKey}`} layout={layout} onZonesChange={setZoneDraft} initialZones={zoneDraft} />
                                    </div>
                                </div>
                            )}
                        </div>
            ...
            {showGalleryModal && (
                <div className="fixed inset-0 z-8000 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
                    <div className={cn(
                        ...
                        theme === 'dark' ? "bg-[#05100e] border border-white/10" : "bg-white border border-slate-200"
                    )}>
                        <div className={cn(
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className={cn("font-bold text-lg", theme === 'dark' ? "text-white" : "text-slate-800")}>Galeria de Mídia</h3>
                            </div>
                            <button 
