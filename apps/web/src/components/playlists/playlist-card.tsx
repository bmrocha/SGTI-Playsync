...
import { Tv, Trash2, Edit, Settings, Link2 } from 'lucide-react';
    ...
    <div className="card-hover bg-panel-bg rounded-xl overflow-hidden shadow-sm border border-border p-4 laptop:p-5 flex flex-col relative group">
      <div className="flex items-start mb-3 laptop:mb-4">
        ...
        <div className="flex-1 min-w-0 pt-0.5 laptop:pt-1">
          <h4
            className="m-0 text-base laptop:text-lg text-text-dark font-bold truncate"
            title={playlist.name}
          >
            {playlist.name}
          </h4>
      ...
      <p className="text-xs laptop:text-sm font-medium text-text-light mb-4 laptop:mb-6 bg-body-bg py-1.5 laptop:py-2 px-2.5 laptop:px-3 rounded-md border border-border inline-flex items-center gap-2 self-start">
        <span>
          🎬 {playlist.items.length} {playlist.items.length === 1 ? 'item' : 'itens'}
        </span>
        {playlist.sharingLinks > 0 && (
          <>
            <span className="text-border">|</span>
            <span
              className="inline-flex items-center gap-1 text-brand-main"
              title={`${playlist.sharingLinks} link(s) de compartilhamento ativo(s)`}
            >
              <Link2 className="w-3 h-3" />
              {playlist.sharingLinks}
            </span>
          </>
        )}
      </p>
