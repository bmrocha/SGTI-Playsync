...
import { Lock, Eye, EyeOff, Keyboard, AlertTriangle, ShieldCheck } from "lucide-react";
...
export function ChangePasswordModal({ isOpen, onClose }: ChangePasswordModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (newPassword === suggestedPassword) {
            setError("Você deve criar sua própria senha. A senha sugerida não pode ser utilizada.");
            return;
        }
        if (!isPasswordValid) {
            setError("A senha deve ter no mínimo 12 caracteres, incluindo maiúscula, minúscula, número e caractere especial.");
            return;
        }
                ...
                >
                    ...
                    >
                        ...
                        <div className="text-center space-y-3 relative z-10">
                            ...
                            <div>
                                <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none mb-2">Redefinir Acesso</h2>
                                <p className="text-slate-400 dark:text-zinc-500 text-[10px] uppercase font-bold tracking-[0.2em]">Protocolo de Segurança Ativo</p>
                            </div>
                        </div>
                            ...
                            <div className="space-y-5">
                                <div className="relative group/input">
                                    <input
                                        ...
                                        className={`w-full px-6 py-4 bg-slate-100/30 dark:bg-white/[0.03] rounded-2xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-zinc-700 focus:outline-none focus:ring-1 transition-all pr-12 ${
                                    ...
                                    />
                                    {newPassword.length > 0 && (
                                        <div className={`absolute right-14 top-1/2 -translate-y-1/2 text-[10px] font-bold ${
                                    ...
                                    <div className="space-y-1.5 px-1">
                                        {passwordRequirements.map((req, i) => (
                                            <div key={i} className="flex items-center gap-2">
                                                ...
                                                <span className={`text-[11px] font-medium ${req.valid ? 'text-emerald-500' : 'text-slate-400 dark:text-zinc-500'}`}>
                                                    {req.label}
                                                </span>
                                            </div>
                                        ))}
                                ...
                                <div className="p-3 bg-emerald-500/5 border border-emerald-500/10 rounded-xl">
                                    <div className="flex items-center justify-between mb-1">
                                        <p className="text-[9px] font-bold text-emerald-500/80 uppercase tracking-wider">Senha sugerida</p>
                                        <button
                                            type="button"
                                    ...
                                    <p className="text-xs font-mono text-emerald-600 dark:text-emerald-400 break-all select-all">{suggestedPassword}</p>
                                    <p className="text-[9px] text-slate-400 dark:text-zinc-500 mt-1">Crie sua própria senha manualmente. Esta sugestão é apenas uma referência.</p>
                                </div>
