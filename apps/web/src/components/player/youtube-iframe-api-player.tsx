...
const loadYoutubeIframeApi = () => {
    if (typeof window === 'undefined') return Promise.reject(new Error('YouTube IFrame API requires a browser environment'));
...
export function YoutubeIFrameApiPlayer({
    ...
    useEffect(() => {
        ...
        loadYoutubeIframeApi()
            .then((YT) => {
                ...
                const start = Number.isFinite(config?.startSeconds as number) ? Math.max(0, Math.floor(config!.startSeconds!)) : undefined;
    ...
    useEffect(() => {
        applyConfig();
    }, [config?.volume, config?.playbackRate, config?.quality, config?.captionsEnabled, config?.captionsLang]);
