...
export function VideoPlayer({
  ...
  // Attempt to unmute after a brief delay, leveraging any recent user-gesture unlock
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const audioTracksLen =
        typeof anyEl.audioTracks?.length === 'number' ? anyEl.audioTracks.length : null;
      const mozHasAudio = typeof anyEl.mozHasAudio === 'boolean' ? anyEl.mozHasAudio : null;
      const webkitAudioBytes =
        typeof anyEl.webkitAudioDecodedByteCount === 'number'
          ? anyEl.webkitAudioDecodedByteCount
          : null;
      ...
      {showControls && (
        ...
        >
          <div className="flex items-center gap-3 w-full">
            <span className="text-xs text-white/80 font-mono min-w-10 text-right">
