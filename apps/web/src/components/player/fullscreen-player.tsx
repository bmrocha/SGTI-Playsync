...
export function FullscreenPlayer({
    ...
    }, [currentIndex, loopCount, isOpen, isPaused, validItems, companyId, playlistId, companyName, playlistName]);
    ...
    let gridItems: ({ type: string; url: string; name: string; id?: string; rotation?: number } | null)[] = [];
    ...
    const renderInnerContent = (type: string, url: string, name: string, rotation: number = 0, regionConfig?: Record<string, unknown>) => {
        const safeUrl = url;
        ...
        if (effectiveType === 'image') {
            return (
                <ImageWithFallback src={safeUrl} alt={name} rotation={rotation} theme={theme} />
            );
        }

        if (effectiveType === 'youtube') {
            const ytConfig = (regionConfig && (regionConfig.__youtube as Partial<YoutubePlayerConfig> | undefined)) || undefined;
        ...
        if (effectiveType === 'web') {
            return <WebFrame src={safeUrl} title={name || "Web Content"} rotation={rotation} theme={theme} />;
        }
    ...
    const renderMediaItem = (item: { type: string; url: string; name: string; rotation?: number; layoutTemplateId?: string; regionConfig?: Record<string, unknown> } | null, index: number) => {
        if (!item || (!item.url && !item.layoutTemplateId)) {
            console.warn('[FullscreenPlayer] Skipping item without URL:', item);
            return (
                <div key={index} className="flex items-center justify-center bg-gray-900 text-white/30 text-sm border border-white/5 w-full h-full">
        ...
        const isVideo = item.type === 'video' || item.type === 'youtube' || (item.url && (
    ...
    const template = currentItem.layoutTemplateId ? getLayoutTemplate(currentItem.layoutTemplateId) : null;
    ...
    return (
        ...
        >
            ...
            <div className={`absolute bottom-8 left-1/2 transform -translate-x-1/2 premium-glass px-8 py-3 rounded-2xl text-xs font-bold tracking-widest uppercase shadow-2xl border border-white/10 transition-all duration-500 ${showControls ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <div className="flex items-center gap-4 text-white/90">
                    <span className="text-brand-main">ITEM {currentIndex + 1} / {validItems.length}</span>
