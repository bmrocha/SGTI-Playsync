...
export function LicenseStatusCard() {
  ...
  const ls = useMemo(() => {
    if (!status) return { active: false, remainingDays: 0, mode: "off" as const, required: false, reason: "" };
    const now = Date.now();
    ...
    return { mode: status.mode, required: status.enforced, active, remainingDays, reason: status.reason || "" };
  }, [status]);

  const copyToClipboard = useCallback(async (text: string, setter: (v: boolean) => void, msg?: string) => {
    try {
  ...
  if (loading) {
    return (
      <div className="bg-white/50 dark:bg-zinc-900/20 border border-zinc-200 dark:border-zinc-800 rounded-[2rem] p-8">
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h2 className="text-2xl font-black text-text-dark dark:text-white tracking-tight">Licenciamento</h2>
              <span className="bg-zinc-100 dark:bg-white/10 text-zinc-500 dark:text-zinc-400 text-[9px] font-black uppercase tracking-[1px] px-3 py-1 rounded-full border border-white/10 flex items-center gap-1.5">
                {ls.mode === "jwt" ? "JWT RS256" : ls.mode === "off" ? "DESATIVADO" : "DESCONHECIDO"}
              </span>
            </div>
            <p className="text-text-light dark:text-white/40 font-bold uppercase tracking-widest text-[9px] mt-1 pl-0.5">
              {ls.mode === "jwt" ? "Licenciamento via token JWT com assinatura RS256" : "Licenciamento não configurado"}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="p-5 rounded-2xl bg-white/50 dark:bg-zinc-900/20 border border-zinc-200 dark:border-zinc-800">
          <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Installation ID</p>
          <p className="text-sm font-bold mt-2 text-text-dark dark:text-white break-all">{status?.installationId || "—"}</p>
        ...
        <div className="p-5 rounded-2xl bg-white/50 dark:bg-zinc-900/20 border border-zinc-200 dark:border-zinc-800">
          ...
          <div className="mt-3 flex items-center gap-3">
            <div className={`w-10 h-10 rounded-2xl border border-zinc-200 dark:border-zinc-700 flex items-center justify-center ${
              ls.active ? "text-emerald-500" : "text-red-500"
            }`}>
              {ls.active ? <CheckCircle2 className="w-5 h-5" /> : <AlertCircle className="w-5 h-5" />}
            </div>
            <div>
              ...
              <p className="text-[10px] text-zinc-400 font-bold mt-0.5">
                Modo: {ls.mode.toUpperCase()} {ls.active ? `• Restante: ${ls.remainingDays} dias` : ""}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-white/50 dark:bg-zinc-900/20 border border-zinc-200 dark:border-zinc-800">
          <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Janela de Licença</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white/50 dark:bg-zinc-900/20 border border-zinc-200 dark:border-zinc-800 rounded-[2rem] p-6 space-y-4">
          <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Suporte e Renovação</p>
          ...
          <div className="space-y-3">
            <p className="text-sm font-bold text-text-dark dark:text-white">Para gerar/renovar sua licença</p>
            <p className="text-[10px] text-zinc-400 leading-relaxed">
              Clique em "Copiar" e envie um e-mail para o suporte com o conteúdo colado no corpo da mensagem.
            </p>
            <div className="p-4 rounded-2xl bg-black/10 dark:bg-white/5 border border-zinc-200 dark:border-zinc-700 space-y-3">
              <p className="text-[9px] font-black uppercase tracking-widest text-zinc-400">Obrigatório</p>
              <button
                onClick={async () => {
                  if (!status?.installationId) return;
                  if (!publicKeyPem) {
                    notifyError("Chave pública indisponível", "A chave pública solicitada não foi encontrada");
                    return;
                  }
                  const msg =
              ...
              <p className="text-[10px] text-zinc-400 leading-relaxed">
                ...
                Destinatário:{" "}
                <span className="font-bold text-text-dark dark:text-white">suporte@sgti.tec.br</span>.
              </p>
            </div>
          </div>
        </div>
