...
import { Play, Tv, Eye, EyeOff, AlertTriangle, Shield, Zap, Activity, Lock, Sun, Moon, ShieldCheck } from "lucide-react";
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFA] dark:bg-[#020a0a] flex items-center justify-center p-4 lg:p-8 overflow-hidden relative font-sans selection:bg-emerald-500/20 text-slate-900 dark:text-slate-200 transition-colors duration-700">
            ...
            <div className="absolute inset-0 z-0">
                ...
                <div className="absolute bottom-[5%] right-[5%] w-[400px] h-[400px] opacity-[0.12] dark:opacity-15 pointer-events-none text-emerald-600 dark:text-emerald-400/30">
                    <Tv className="w-full h-full animate-pulse" />
                </div>

                {/* Balanced Ambient Glow */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-emerald-500/[0.08] dark:bg-emerald-500/[0.05] rounded-full blur-[180px]"></div>
                <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-emerald-600/[0.05] dark:bg-emerald-500/[0.03] rounded-full blur-[150px]"></div>
            </div>

            {/* 2. DISCREET SCAN LINE */}
            <div
                className="absolute left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-emerald-500/20 dark:via-emerald-500/30 to-transparent z-10 pointer-events-none"
            ...
            <div className="relative z-20 w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                ...
                >
                    <div className="flex flex-col md:flex-row md:items-center gap-8 md:gap-12">
                        ...
                        {logoLoginUrl && (
                            <div className="flex-shrink-0 animate-in fade-in slide-in-from-right-4 duration-1000">
                                <Image
                                    src={logoLoginUrl}
                                    alt="System Logo"
                                    width={0}
                        ...
                        <div className="space-y-4">
                            ...
                            <h1 className="text-5xl md:text-7xl font-black tracking-tighter leading-none text-slate-900 dark:text-white transition-colors flex items-center gap-1">
                                ...
                                <span className="text-emerald-500/60 dark:text-emerald-500/50 whitespace-nowrap">{systemName === 'PlaySync' ? 'Sync' : (systemName && systemName.includes(' ') ? systemName.split(' ').slice(1).join(' ') : 'Sync')}</span>
                            </h1>
                        </div>
                    </div>
                ...
                >
                    <div className={`w-full max-w-[380px] bg-white/60 dark:bg-[#0c0e12]/80 backdrop-blur-3xl border border-black/5 dark:border-white/5 rounded-[3rem] p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] dark:shadow-2xl relative transition-all duration-700 ${isLoading ? 'ring-2 ring-emerald-500/50' : ''}`}>

                        <div className="text-center mb-8">
                            {logoLoginUrl ? (
                                <img src={logoLoginUrl} alt="Logo" className="h-16 w-16 object-contain mx-auto mb-6 opacity-90 drop-shadow-lg" />
                            ) : (
                            ...
                            <h2 className="text-xl font-bold text-slate-900 dark:text-white uppercase tracking-[0.2em] transition-colors">Autenticação</h2>
                            <p className="text-[9px] text-emerald-600 dark:text-emerald-500/40 font-black uppercase tracking-[0.3em] mt-1 transition-colors">Secured Data Access</p>
                        </div>
                        ...
                        {isTwoFactorStep ? (
                            ...
                            {error && (
                                ...
                                >
                                    ...
                                    <motion.div
                                        className="absolute inset-0 bg-gradient-to-r from-transparent via-white/40 dark:via-black/20 to-transparent -translate-x-full"
                                        animate={isLoading ? { x: ['100%', '-100%'] } : {}}
                                        transition={{ repeat: Infinity, duration: 1, ease: "linear" }}
                                    />

                                    <div className="absolute inset-0 bg-gradient-to-tr from-emerald-400/0 via-emerald-400/20 to-emerald-400/0 opacity-0 group-hover/btn:opacity-100 transition-opacity duration-500" />
                                </button>
                                
                                <button 
                                    type="button" 
                                    onClick={() => { setIsTwoFactorStep(false); setTempToken(""); setError(""); }}
                        ...
                        <div className="mt-8 text-center opacity-20 hover:opacity-100 transition-opacity">
                            <p className="text-[8px] font-mono uppercase tracking-widest">v4.5.1 // Protocol: 256-Bit</p>
                        </div>
                            </>
                        )}
                    </div>
