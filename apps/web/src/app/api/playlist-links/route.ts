...
import { CompanyRepository, PlaylistRepository, CompanyPlaylistRepository, MediaItemRepository, PlaylistLinkRepository } from '@playsync/database';
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (!hasPermission(currentUser.role as UserRole, Permission.EDIT_PLAYLIST)) {
        ...
        if (playlistId) {
            ...
            if (!resolvedCompanyId && Array.isArray((playlist as any).companies) && (playlist as any).companies.length > 0) {
            ...
            if (link) {
                ...
                console.log(`[API] Found existing link ${link.id} for playlist ${playlist.id}. Updating theme=${theme}, color=${primaryColor}`);
                ...
                link = await PlaylistLinkRepository.create(id, resolvedCompanyId as string, playlist.id, theme || 'light', primaryColor);
            }
        ...
        if (link) {
            ...
            link = await PlaylistLinkRepository.create(id, company.id, playlist.id, theme || 'light', primaryColor);
        }
