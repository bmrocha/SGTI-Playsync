...
const ALTERNATE_UPLOAD_DIR = path.join(process.cwd(), 'apps', 'web', 'public', 'uploads');
...
function resolveUploadDir(): string {
    ...
    if (fs.existsSync(path.dirname(ALTERNATE_UPLOAD_DIR))) {
        return ALTERNATE_UPLOAD_DIR;
    }
    return DEFAULT_UPLOAD_DIR;
}
...
async function ensureMediaLibraryTable() {
    ...
    try {
        await query(`CREATE INDEX IF NOT EXISTS idx_media_library_uploaded_at ON media_library(uploaded_at DESC);`);
    } catch {
    }
    try {
        await query(`CREATE INDEX IF NOT EXISTS idx_media_library_mime_type ON media_library(mime_type);`);
    } catch {
    }
}
...
const MAGIC_BYTES: Record<string, number[][]> = {
    ...
    '.mp4': [[0x00, 0x00, 0x00], [0x66, 0x74, 0x79, 0x70]], // ftyp box (offset 4)
    '.webm': [[0x1A, 0x45, 0xDF, 0xA3]],
    '.mov': [[0x00, 0x00, 0x00], [0x66, 0x74, 0x79, 0x70]], // ftyp box (offset 4)
};
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const isVideo = file.type?.startsWith('video') || ['.mp4', '.webm', '.mov', '.mkv', '.m4v', '.avi', '.wmv', '.flv', '.ogg'].includes(extLower);
        ...
        if (!validTypes) {
            ...
            return NextResponse.json(
                { error: 'Tipo de arquivo nao permitido. Apenas imagens (jpg, png, webp, gif) e videos (mp4, webm, mov) sao aceitos.' },
                { status: 400 }
            );
        }
