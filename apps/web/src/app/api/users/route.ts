...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const { users: rawUsers, total } = await UserRepository.findAll(companyId, page, limit, search, role);
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (!hasPermission(currentUser.role as UserRole, Permission.MANAGE_USERS)) {
            return NextResponse.json(
...
export async function PUT(request: NextRequest) {
    try {
        ...
        const { id, email, name, role, password, avatar, forcePasswordReset, twoFactorEnabled, forceTwoFactorSetup, theme, primaryColor } = result.data;

        const isSelfUpdate = currentUser.id === id;
        const isAdmin = hasPermission(currentUser.role as UserRole, Permission.MANAGE_USERS);
        ...
        if (originalUser.email === 'admin@sgti.tec.br' || originalUser.email === 'admin@sgti.com') {
             if (currentUser.email !== originalUser.email) {
                return NextResponse.json(
                    { error: 'Segurança do Sistema: O usuário Administrador (Root) não pode ser modificado por outros.' },
                    { status: 403 }
                );
             }
        }
        ...
        if (twoFactorEnabled !== undefined) {
            if (isAdmin) {
                ...
                const isRootUser = originalUser.email === 'admin@sgti.tec.br' || originalUser.email === 'admin@sgti.com';
                if (isRootUser && twoFactorEnabled === false) {
                     return NextResponse.json(
                        { error: 'Segurança: Não é permitido desativar a autenticação de dois fatores para o usuário Root.' },
                        { status: 403 }
                    );
                }
        ...
        if (passwordChanged) {
            auditPromises.push(logServerAction({
                ...baseAuditParams,
                action: 'user_updated',
        ...
        if (forcePasswordReset !== undefined && originalUser.force_password_reset !== forcePasswordReset) {
        ...
        const otherChanges = [];
        if (name && name !== originalUser.name) otherChanges.push(`Nome: ${originalUser.name} -> ${name}`);
        if (email && email.toLowerCase() !== originalUser.email) otherChanges.push(`Email: ${originalUser.email} -> ${email}`);
