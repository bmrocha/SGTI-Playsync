...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (
      !hasPermission(currentUser.role as UserRole, Permission.VIEW_PLAYER, currentUser.permissions)
    ) {
    ...
    let companyId =
      currentUser.role === UserRole.ADMIN ? undefined : currentUser.companyId || undefined;
    ...
    const { players: rawPlayers, total } = await PlayerRepository.findAllPaginated(
      companyId,
      page,
      limit,
      search,
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    if (
      !hasPermission(currentUser.role as UserRole, Permission.EDIT_PLAYER, currentUser.permissions)
    ) {
