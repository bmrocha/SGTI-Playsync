...
const auditLogSchema = z.object({
    ...
    resource: z.enum(['auth', 'user'], { message: 'resource must be "auth" or "user"' }),
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const filters = auditFilterSchema.safeParse({
            ...
            resources: searchParams.get('resources') ? searchParams.get('resources')!.split(',') : undefined,
        ...
        const { userId: filterUserId, action: filterAction, resource: filterResource, resources, startDate, endDate, page = 1, limit = 50 } = filters.data;
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const forwardedFor = request.headers.get('x-forwarded-for') || request.headers.get('x_forwarded_for');
