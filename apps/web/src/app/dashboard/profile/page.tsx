...
import { User, Mail, Shield, Calendar, Camera, ArrowLeft, Save, Loader2, X, Check, Palette, Moon, Sun, Lock, Key, Eye, EyeOff } from "lucide-react";
...
function SettingsPageContent() {
    ...
    const { theme, primaryColor, toggleTheme, setTheme, setPrimaryColor } = useThemeStore();
    ...
    useEffect(() => {
        const handleClickOutside = (e: MouseEvent) => {
            if (hasChanges && rootRef.current && !rootRef.current.contains(e.target as Node)) {
                ...
                const isHeaderClick = (e.target as HTMLElement).closest('.header-container') || 
    ...
    useEffect(() => {
        ...
        setHasChanges(isNameChanged || isEmailChanged || isAvatarChanged || isThemeChanged || isColorChanged || isPasswordChanged);
    }, [name, email, previewAvatar, user, draftTheme, draftPrimaryColor, initialTheme, initialPrimaryColor, password]);
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            if (file.size > 5 * 1024 * 1024) {
                notifyError("A imagem deve ter no máximo 5MB", "O arquivo selecionado excede o limite de 5MB");
                return;
            }
    ...
    const handleSave = async () => {
        ...
        try {
            if (password && password !== confirmPassword) {
                notifyError("As senhas não coincidem.", "Os campos de nova senha e confirmação devem ser iguais");
                setIsLoading(false);
                return;
            }
            if (password) {
                ...
                if (!reqs.every(r => r)) {
                    notifyError("A senha deve ter no mínimo 12 caracteres, incluindo maiúscula, minúscula, número e caractere especial.", "");
                    setIsLoading(false);
                    return;
                }
            }
            ...
            notifyError(error.message || "Erro ao salvar alterações", "Tente novamente ou contate o suporte");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div ref={rootRef} className="py-2.5 px-4 sm:px-6 lg:px-8 flex flex-col gap-3 w-full animate-fadeIn h-full overflow-hidden">
            ...
            <div className="shrink-0 border-b border-slate-200 pb-3 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-text-light text-xs mt-0.5">Gerencie suas informações e preferências.</p>
                </div>
                    ...
                    <div className="bg-panel-bg rounded-2xl shadow-sm border border-border overflow-hidden flex flex-col flex-1">
                        <div className="h-20 bg-linear-to-r from-brand-main to-brand-accent relative shrink-0">
                            <div className="absolute -bottom-10 left-1/2 -translate-x-1/2">
                                ...
                                >
                                    {previewAvatar ? (
                                        <Image src={previewAvatar} alt={name} fill className="object-cover" unoptimized />
                                    ) : (
                        ...
                        <div className="pt-12 pb-4 px-4 text-center flex flex-col gap-3">
                            ...
                            <div className="p-3 bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-lg flex gap-2 text-left">
                                ...
                                <div>
                                    <h4 className="text-xs font-bold text-blue-700 dark:text-blue-400">Conta Ativa</h4>
                        ...
                        <div className="bg-panel-bg rounded-2xl shadow-sm border border-border p-6 flex flex-col gap-4 flex-1">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 py-2">
                                ...
                                <div className="space-y-1.5">
                                    <label className="block text-[10px] font-black text-text-light uppercase tracking-wider">Nome Completo</label>
                                ...
                                {/* Email (Read-only) */}
                                <div className="space-y-1.5">
                                    <label className="block text-[10px] font-black text-text-light uppercase tracking-wider">Endereço de Email</label>
                                    ...
                                    <p className="text-[9px] text-text-light/70 ml-1 font-medium italic">E-mail gerenciado pelo administrador master.</p>
                                </div>

                                {/* Nível */}
                                <div className="space-y-1.5">
                                    <label className="block text-[10px] font-black text-text-light uppercase tracking-wider">Nível de Acesso</label>
                                    <div className="flex items-center gap-3 p-3 bg-body-bg/50 border border-border rounded-xl opacity-60 relative">
                                        ...
                                        <span className="font-bold text-sm text-text-dark capitalize tracking-tight">{user.role}</span>
                                        <Lock className="w-3.5 h-3.5 text-text-light absolute right-3 opacity-40" />
                                    </div>
                                </div>
                                ...
                                {/* Data de Criação */}
                                <div className="space-y-1.5">
                                    <label className="block text-[10px] font-black text-text-light uppercase tracking-wider">Membro da Rede Desde</label>
                                    <div className="flex items-center gap-3 p-3 bg-body-bg/50 border border-border rounded-xl opacity-60 relative">
                                        <Calendar className="w-4 h-4 text-text-light" />
                                        <span className="font-bold text-sm text-text-dark">{new Date(user.createdAt).toLocaleDateString()}</span>
                                        <Lock className="w-3.5 h-3.5 text-text-light absolute right-3 opacity-40" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mt-4 p-5 bg-panel-bg border border-border rounded-xl flex flex-col gap-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                                    {/* Nova Senha */}
                                    <div className="space-y-1.5">
                                        <label className="block text-[10px] font-black text-text-light uppercase tracking-wider">Nova Senha</label>
                                        <div className={`flex items-center gap-3 p-3 bg-body-bg border rounded-xl transition-all group relative ${
                                            password.length === 0
                                            ...
                                            />
                                            {password.length > 0 && (
                                                <span className={`absolute right-10 text-[10px] font-bold ${
                                            ...
                                            <div className="space-y-1 pt-1">
                                                {[
                                                    ...
                                                    <div key={i} className="flex items-center gap-1.5">
                                                        ...
                                                        <span className={`text-[10px] font-medium ${req.valid ? 'text-emerald-500' : 'text-text-light'}`}>
                                                            {req.label}
                                                        </span>
                                                    </div>
                                                ))}
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="block text-[10px] font-black text-text-light uppercase tracking-wider">Confirmar Nova Senha</label>
                                        <div className={`flex items-center gap-3 p-3 bg-body-bg border rounded-xl transition-all group relative ${
                                        ...
                                        {confirmPassword.length > 0 && (
                                            <div className="flex items-center gap-1.5 pt-1">
                                                ...
                                                <span className={`text-[10px] font-medium ${confirmPassword === password && password.length >= 12 ? 'text-emerald-500' : 'text-text-light'}`}>
                                                    Senhas coincidem
                                                </span>
                                            </div>
                                        )}
                            ...
                            <div className="pt-4 border-t border-border mt-auto shrink-0">
                                <button
                                    onClick={handleSave}
                                    disabled={!hasChanges || isLoading || (password.length > 0 && (!(password.length >= 12 && /[A-Z]/.test(password) && /[a-z]/.test(password) && /[0-9]/.test(password) && /[^A-Za-z0-9]/.test(password)) || password !== confirmPassword))}
                                ...
                                >
                                    {isLoading ? (
                                        ...
                                        <><Save className="w-4 h-4 mr-2" />Confirmar Alterações de Perfil</>
                                    )}
                                </button>
                            </div>
                        </div>
...
export default function SettingsPage() {
    return (
        <Suspense fallback={<div className="flex items-center justify-center h-full"><Loader2 className="w-8 h-8 animate-spin text-brand-main" /></div>}>
            <SettingsPageContent />
        </Suspense>
    );
}
    ...
    return (
        <div className="bg-panel-bg rounded-2xl shadow-sm border border-border p-6 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label className="block text-[10px] font-bold text-text-light uppercase tracking-wider mb-2">Tema</label>
                ...
                <div>
                    <label className="block text-[10px] font-bold text-text-light uppercase tracking-wider mb-2">Cor Principal</label>
                    <div className="flex items-center gap-2">
                        {colors.map((color) => (
                                ...
                                >
                                    ...
                                    {color === "#11876d" && (
                                        <div className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-[#d4ff00] rounded-full border border-black/20 neon-pulse" title="Cor padrão PlaySync" />
                                    )}
                                </button>
                        ))}
                    </div>
