...
import { Users, Shield, Mail, Trash2, Edit, Plus, Search, UserCog, Key, Filter, X, Zap, Eye, FileEdit, Building2 } from "lucide-react";
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 pb-6 border-b border-slate-200">
                <div>
                    <h1 className="text-3xl font-bold text-text-dark flex items-center gap-3">
                        <UserCog className="w-8 h-8 text-brand-main" />
                        Gerenciar Usuários
                    </h1>
                    <p className="text-text-light mt-2">{stats.total} usuários cadastrados</p>
                </div>
                ...
                <div className="card-hover bg-panel-bg p-5 laptop:p-4 rounded-xl border border-border">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-3xl laptop:text-2xl font-bold text-text-dark mt-1">{stats.total}</p>
                        </div>
                ...
                <div className="card-hover bg-panel-bg p-5 laptop:p-4 rounded-xl border border-border">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-3xl laptop:text-2xl font-bold text-text-dark mt-1">{stats.admins}</p>
                        </div>
                ...
                <div className="card-hover bg-panel-bg p-5 laptop:p-4 rounded-xl border border-border">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-3xl laptop:text-2xl font-bold text-text-dark mt-1">{stats.editors}</p>
                        </div>
                ...
                <div className="card-hover bg-panel-bg p-5 laptop:p-4 rounded-xl border border-border">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-3xl laptop:text-2xl font-bold text-text-dark mt-1">{stats.viewers}</p>
                        </div>
                    ...
                    <table className="w-full">
                        <thead className="bg-border/30">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-left text-xs font-bold text-text-light uppercase tracking-wider">Função</th>
                                <th className="px-6 py-4 text-left text-xs font-bold text-text-light uppercase tracking-wider">Criado em</th>
                                <th className="px-6 py-4 text-left text-xs font-bold text-text-light uppercase tracking-wider">Último Acesso</th>
                                {canManageUsers && (
                                    <th className="px-6 py-4 text-right text-xs font-bold text-text-light uppercase tracking-wider">Ações</th>
                                )}
                            </tr>
                        </thead>
                                            ...
                                            <div
                                                ...
                                                style={{
                                                    backgroundColor: !user.avatar
                                                        ? (user.email === 'admin@sgti.com' ? '#06b6d4' : (roleConfig[user.role as UserRole]?.color || '#ccc'))
                                                        : 'transparent'
                                                }}
                                            >
                                                {user.avatar ? (
                                        ...
                                        <div>
                                            <span
                                                ...
                                                style={{
                                                    ...
                                                    color: user.email === 'admin@sgti.com' ? '#06b6d4' : (roleConfig[user.role as UserRole]?.color || '#ccc')
                                                }}
                                            >
                                                {(() => {
                                                    const Icon = user.email === 'admin@sgti.com' ? Zap : (roleConfig[user.role as UserRole]?.icon || Shield);
                                                    return <Icon className="w-3 h-3 mr-1" />;
                                                })()}
                                                {user.email === 'admin@sgti.com' ? 'Hoot' : (roleConfig[user.role as UserRole]?.label || user.role)}
                                            </span>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex justify-end gap-2">
                                                {user.email === 'admin@sgti.com' ? (
                                                    <div className="flex items-center text-[10px] font-bold p-2 gap-1 italic opacity-80" style={{ color: '#06b6d4' }}>
