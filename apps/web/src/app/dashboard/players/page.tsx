...
export default function PlayersPage() {
  ...
  const fetchPlayers = useCallback(async () => {
    ...
    try {
      ...
      notifyError(
        ...
        'Não foi possível obter a lista de players do servidor',
      );
    } finally {
  ...
  return (
    <div className="p-4 laptop:p-6">
        ...
        <div className="mb-6 p-5 bg-panel-bg rounded-xl border border-border animate-slideDown shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="relative">
              <label className="text-xs font-semibold text-text-light mb-1.5 block uppercase">
                Buscar
              </label>
              <div className="relative">
                <input
      ...
      {players.length === 0 && !isRefreshing ? (
        <div className="text-center py-20 bg-panel-bg rounded-2xl border-2 border-dashed border-border">
          ...
          {totalItems === 0 &&
            ...
            companyFilter === 'all' &&
            user &&
            hasPermission(user.role as UserRole, Permission.CREATE_PLAYER) && (
      ...
      <NewPlayerModal isOpen={isModalOpen} onClose={handleCloseModal} initialData={editingPlayer} />
    </div>
  );
}
