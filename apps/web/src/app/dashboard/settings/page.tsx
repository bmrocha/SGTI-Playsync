...
import {
    Settings, Save, Loader2, ChevronRight,
    Video, Lock, Server, KeyRound
} from "lucide-react";
...
export default function SystemSettingsPage() {
    ...
    const {
        systemName, setField, footerMessage, logoSidebarUrl, logoLoginUrl, logoAuthUrl, faviconUrl,
        isMaintenanceMode, maintenanceMessage, maintenanceEstimatedTime, maintenancePriority,
        maintenancePages, maintenanceScope,
        isBruteForceProtection, isTwoFactorEnforced, auditLogRetention, isPasswordComplexityEnforced,
        sessionLimit, restrictIP, trusted_ips, webhookUrl, isDebugMode, isAutoClean, isAutoBackup,
        cleanDays, logLevel, storageLimit,
        updateMetrics,
        autoPlayVideos, hardwareAccel, mediaCacheSize, isOfflineSyncEnabled,
        isAiUpscaling, isHdrEnabled, isAudioNormalized, isP2pEnabled, mediaQuality, uploadLimit, uploadLimitVideo,
        fetchSettings
    } = useSystemStore();
    ...
    const [maintenanceScopeLocal, setMaintenanceScopeLocal] = useState<'site' | 'pages'>(maintenanceScope ?? 'site');
    const [maintenancePagesLocal, setMaintenancePagesLocal] = useState<string[]>(maintenancePages ?? []);
    ...
    const memUsedGb = typeof telemetry?.memory?.system?.usedBytes === 'number'
        ? telemetry.memory.system.usedBytes / 1024 / 1024 / 1024
        : 0;
    const memUsedPercent = typeof telemetry?.memory?.system?.usedPercent === 'number' ? telemetry.memory.system.usedPercent : 0;
    const storageUsedPercent = typeof telemetry?.mediaStorage?.usedPercent === 'number' ? telemetry.mediaStorage.usedPercent : 0;
    const storageLimitLabel = typeof telemetry?.mediaStorage?.limitBytes === 'number' && telemetry.mediaStorage.limitBytes > 0
        ? `${Math.round(telemetry.mediaStorage.limitBytes / 1024 / 1024)}MB`
        : '—';
    const dbLatencyLabel = typeof telemetry?.db?.latencyMs === 'number' ? `${telemetry.db.latencyMs}ms` : '—';
    ...
    const hasChanges = useMemo(() => {
        ...
        const originalNormalized = {
            ...
            maintenancePages: JSON.stringify([...(Array.isArray((originalSettings as any).maintenancePages) ? (originalSettings as any).maintenancePages : [])].sort()),
        ...
        systemName, footerMessage, logoSidebarUrl, logoLoginUrl, logoAuthUrl, faviconUrl,
        isAutoBackup, isAutoClean, cleanDays, sessionLimit, restrictIP,
        uploadLimit, uploadLimitVideo, mediaQuality, autoPlayVideos, webhookUrl,
        maintenanceMessage, maintenanceEstimatedTime, maintenancePriority,
        isBruteForceProtection, isTwoFactorEnforced, auditLogRetention,
        isPasswordComplexityEnforced, isMaintenanceModeLocal, maintenanceScopeLocal, maintenancePagesLocal, isDebugMode, logLevel, storageLimit,
        hardwareAccel, isAiUpscaling, isHdrEnabled, isAudioNormalized,
        isP2pEnabled, isOfflineSyncEnabled, originalSettings, mediaCacheSize
    ]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setOriginalSettings(initialData);
                notifySuccess("Configurações globais salvas no servidor!", "Todas as alterações foram aplicadas");
            } else {
    ...
    const tabs = [
        { id: "midia", label: "Mídia & Player", icon: Video, color: "text-zinc-400", bg: "bg-zinc-400/10" },
        { id: "seguranca", label: "Segurança & Acesso", icon: Lock, color: "text-zinc-400", bg: "bg-zinc-400/10" },
        { id: "infra", label: "Infraestrutura & Ops", icon: Server, color: "text-zinc-400", bg: "bg-zinc-400/10" },
        { id: "sistema", label: "Sistema", icon: KeyRound, color: "text-zinc-400", bg: "bg-zinc-400/10" },
    ];
                ...
                <div className="flex items-center gap-4">
                    {hasChanges && (
                        <div className="hidden lg:flex items-center mr-6 animate-fadeIn">
                            <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-zinc-400/10 text-zinc-600 dark:text-amber-400 text-[10px] font-black uppercase tracking-widest animate-pulse border border-zinc-400/20 ">
                    ...
                    <button
                        ...
                        className={`min-w-[150px] px-5 py-2 rounded-xl transition-all duration-300 flex items-center justify-center gap-2.5 font-black group h-10 relative overflow-hidden  ${hasChanges
                    ...
                    >
                        <div className="relative z-20 flex items-center gap-2">
                            {isLoading ? (
                                ...
                                <Save className={`w-3.5 h-3.5 transition-all duration-500 ${hasChanges ? "text-white" : "text-gray-400"}`} />
                            )}
                            <span className={`text-[10px] uppercase tracking-widest font-black ${hasChanges ? "text-white" : "text-gray-500"}`}>
                    ...
                    {tabs.map((tab) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center justify-between p-2.5 rounded-xl transition-all duration-300 group relative overflow-hidden ${isActive
                                    ? `bg-white dark:bg-white/5  scale-[1.02] z-10`
                                    : 'hover:bg-white/50 dark:hover:bg-white/5 text-text-light'
                                    }`}
                            >
                                {/* Solid Vertical Indicator */}
                                <div className={`absolute left-0 top-0 bottom-0 w-1.5 transition-all duration-500 ease-in-out ${tab.color.replace('text-', 'bg-')} ${isActive
                                    ? 'opacity-100 h-full'
                                    : 'opacity-0 h-0'
                                    }`} />
                                ...
                                {isActive && (
                                    <div className={`absolute inset-0 opacity-[0.08] ${tab.color.replace('text-', 'bg-')}`} />
                                )}
                                ...
                                <div className="flex items-center gap-2.5 pl-2.5 relative z-10">
                                    <div className={`p-2 rounded-lg transition-all duration-300 ${isActive
                                        ...
                                        <Icon className={`w-4 h-4 transition-transform duration-500 ${isActive ? tab.color : 'text-text-light'}`} />
                                    </div>
                                    <span className={`font-bold text-[13px] transition-all duration-300 ${isActive ? 'text-text-dark' : 'group-hover:text-text-dark'}`}>
                                        {tab.label}
                                    </span>
                                </div>
