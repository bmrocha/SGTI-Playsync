...
export class PlaylistLinkRepository {
    ...
    static async findByPlaylistId(playlistId: string): Promise<PlaylistLink | null> {
        const res = await query('SELECT * FROM playlist_links WHERE playlist_id = $1 ORDER BY created_at ASC LIMIT 1', [playlistId]);
        return res.rows[0] || null;
    }

    static async findByCompanyAndPlaylist(companyId: string, playlistId: string): Promise<PlaylistLink | null> {
    ...
    static async create(id: string, companyId: string, playlistId: string, theme: string = 'light', primaryColor?: string): Promise<PlaylistLink> {
        const res = await query(
    ...
    static async updateTheme(id: string, theme: string): Promise<void> {
        await query('UPDATE playlist_links SET theme = $1, updated_at = NOW() WHERE id = $2', [theme, id]);
    }

    static async updatePrimaryColor(id: string, primaryColor: string): Promise<void> {
        await query('UPDATE playlist_links SET primary_color = $1, updated_at = NOW() WHERE id = $2', [primaryColor, id]);
    }

    static async updateThemeAndColor(id: string, theme: string, primaryColor: string | null): Promise<void> {
        await query(
