...
const USER_ALLOWED_COLUMNS = new Set([
  'email', 'password', 'name', 'role', 'company_id', 'avatar',
  'theme', 'primary_color', 'force_password_reset',
  'two_factor_enabled', 'two_factor_secret', 'two_factor_temp_secret',
  'two_factor_setup_expires', 'force_2fa_setup',
]);
...
export class UserRepository {
  ...
  static async resetFailedLogin(id: string): Promise<void> {
    await query('UPDATE users SET failed_login_attempts = 0, lockout_until = NULL WHERE id = $1', [id]);
  }
  ...
  static async create(data: { email: string; password?: string; name: string; role: string; company_id?: string; force_password_reset?: boolean; force_2fa_setup?: boolean }): Promise<User> {
    const { email, password, name, role, company_id, force_password_reset = false, force_2fa_setup = false } = data;
    const res = await query(
      ...
      [email, password || '', name, role, company_id || null, force_password_reset, force_2fa_setup]
    );
    return res.rows[0];
  }
  ...
  static async findAll(companyId?: string, page: number = 1, limit: number = 10, search?: string, role?: string): Promise<{ users: User[], total: number }> {
