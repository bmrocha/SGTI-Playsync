...
const PLAYLIST_ALLOWED_COLUMNS = new Set([
  'name', 'description',
]);

export class PlaylistRepository {
    static async findAll(companyId?: string, page: number = 1, limit: number = 10, search?: string): Promise<{ playlists: Playlist[], total: number }> {
    ...
    static async create(data: { name: string; description?: string; companyIds?: string[] }): Promise<Playlist> {
        ...
        try {
            ...
            if (companyIds.length > 0) {
                for (const companyId of companyIds) {
                    await query(
                        'INSERT INTO company_playlists (company_id, playlist_id) VALUES ($1, $2)',
                        [companyId, playlist.id]
                    );
                }
            }
    ...
    static async update(id: string, data: Partial<Playlist> & { companyIds?: string[], items?: any[] }): Promise<Playlist | null> {
        try {
            await query('BEGIN');

            // 1. Update Playlist fields
            const fields = Object.keys(data).filter(key => 
                key !== 'id' && key !== 'created_at' && key !== 'updated_at' && key !== 'companyIds' && key !== 'items' && key !== 'companyNames' && PLAYLIST_ALLOWED_COLUMNS.has(key)
            );
            ...
            if (fields.length > 0) {
                ...
                await query(
                    `UPDATE playlists SET ${setClause}, updated_at = NOW() WHERE id = $1`,
                    values
                );
            }
    ...
    static async filterValidItems(items: any[]): Promise<any[]> {
        ...
        for (const item of items) {
            if (item.type === 'youtube' || item.type === 'web' || item.type === 'widget' || item.type === 'layout') {
                valid.push(item);
                continue;
            }
